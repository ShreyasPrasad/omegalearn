...
def check_for_missing_views_table(error):
    """
    Determines if the error is caused by a missing "views" table.
    ...
    """
    return '"views" does not exist' in str(error)


def render_suggestions_for_missing_views_table(error, movr):
    """
    Renders 'display_error.html' template for a missing views table.
    """
    title = "Cannot load page"
    reason = 'Caught Exception: sqlalchemy.exc.ProgrammingError'
    context = "This occured because there is no `views` table."
    possible_sources = ["You may be connected to the wrong database, hence "
                        "the missing table. Your database may not have the "
                        "`views` table for some reason."]
...
def render_error_page(error, movr):
    ...
    """
    if check_for_missing_views_table(error):
        # This can happen if you haven't run `dbinit.sql`
        return render_suggestions_for_missing_views_table(error, movr)
